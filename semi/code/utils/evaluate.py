...
def evaluate(model, dataloader, total_batch):  # 原本有netD参数，但在evaluate函数中并没有使用它，所以删除netD参数
    ...
    IoU_mean = 0
    dice_sum=0
    list_name=[]  # 用于存储每个样本的名称，后续可以根据需要保存到文件或进行分析
    list_point=[]  # 用于存储每个样本的IoU_mean值，后续可以根据需要保存到文件或进行分析
    with torch.no_grad():
